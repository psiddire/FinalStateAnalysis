...
mu_trackingEta_2017 = GraphReaderTrackingEta(
    os.path.join(os.environ['fsa'], 'TagAndProbe/data/2017/fits_muon_trk_2017.root'), 'ratio_eff_eta3_dr030e030_corr'
)
...
#ID Iso
mIDIso = GraphReaderMuonSF(os.path.join(os.environ['fsa'], 'TagAndProbe/data/2016/Muon_Run2016_legacy_IdIso.root'))
...
_DATA_FILES = {
    ...
    '2017' : {
        'ID'   : os.path.join(_DATA_DIR_2017, 'RunBCDEF_SF_ID.root'),
        'Iso'    : os.path.join(_DATA_DIR_2017, 'RunBCDEF_SF_ISO.root'),
        'Trigger': os.path.join(_DATA_DIR_2017, 'EfficienciesAndSF_RunBtoF_Nov17Nov2017.root')
    },
    '2016' : {
        ...
        'Trigger' : [os.path.join(_DATA_DIR_2016, 'EfficienciesAndSF_RunBtoF.root'), os.path.join(_DATA_DIR_2016, 'EfficienciesAndSF_RunGtoH.root')]
    }
}
...
# 2017
def make_muon_pog_PFTight_2017():
    return MuonPOGCorrectionID2D(
        _DATA_FILES['2017']['ID'],
        "NUM_TightID_DEN_genTracks_pt_abseta"
    )
def make_muon_pog_PFMedium_2017():
    return MuonPOGCorrectionID2D(
        _DATA_FILES['2017']['ID'],
        "NUM_MediumID_DEN_genTracks_pt_abseta"
    )
def make_muon_pog_PFLoose_2017():
    return MuonPOGCorrectionID2D(
        _DATA_FILES['2017']['ID'],
        "NUM_LooseID_DEN_genTracks_pt_abseta"
    )
def make_muon_pog_TightIso_2017(MuonID):
    return MuonPOGCorrectionTightIso2D(
        _DATA_FILES['2017']['Iso'], MuonID,
        ["NUM_TightRelIso_DEN_MediumID_pt_abseta",
         "NUM_TightRelIso_DEN_TightIDandIPCut_pt_abseta"]
    )
def make_muon_pog_LooseIso_2017(MuonID):
    return MuonPOGCorrectionLooseIso2D(
        _DATA_FILES['2017']['Iso'], MuonID,
        ["NUM_LooseRelIso_DEN_LooseID_pt_abseta",
         "NUM_LooseRelIso_DEN_MediumID_pt_abseta",
...
def make_muon_pog_IsoMu27_2017():
    return MuonPOGCorrectionTrig2D_2017(
        _DATA_FILES['2017']['Trigger'],
        "IsoMu27_PtEtaBins/pt_abseta_ratio"
    )
def make_muon_pog_Mu50_2017():
    return MuonPOGCorrectionTrig2D_2017(
        _DATA_FILES['2017']['Trigger'],
        "Mu50_PtEtaBins/pt_abseta_ratio"
    )
# 2016
def make_muon_pog_PFTight_2016():
    return MuonPOGCorrectionID2D_2016(
        _DATA_FILES['2016']['ID'],
...
def make_muon_pog_Mu50_2016():
    return MuonPOGCorrectionTrig2D_weighted_2016(
        _DATA_FILES['2016']['Trigger'],
        ["Mu50_OR_TkMu50_PtEtaBins/pt_abseta_ratio", 
         "Mu50_OR_TkMu50_PtEtaBins/pt_abseta_ratio"], 
        20.182, 16.146
    )
...
class MuonPOGCorrectionTrig2D_2017(object):
    def __init__(self, file, pt_abseta):
        self.filename = file
        self.file = ROOT.TFile.Open(file)
        self.histopath = pt_abseta
...
class MuonPOGCorrectionTrig2D_weighted_2016(object):
    def __init__(self, file, pt_abseta, lumi1, lumi2):
        self.file1 = ROOT.TFile.Open(file[0])
        self.file2 = ROOT.TFile.Open(file[1])
        self.lumi1 = lumi1
...
if __name__ == "__main__":
    ...
    make_muon_pog_PFTight_2017()
    make_muon_pog_PFMedium_2017()
    make_muon_pog_PFLoose_2017()
    make_muon_pog_TightIso_2017()
    make_muon_pog_LooseIso_2017()
    make_muon_pog_IsoMu27_2017()
    make_muon_pog_Mu50_2017()
